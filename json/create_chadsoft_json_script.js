...
for (let em of document.getElementsByTagName("tr")) {
	...
	let name = text.innerHTML.replaceAll(" ","").replace("-Normal","").replace("-No-shortcut","").replace("-Shortcut","_sc").replace("-Glitch","_g").replace("GCN","r").replace("GBA","r").replace("DS","r").replace("SNES","r").replace("N64","r").replace("Luigi","l").replace("Circuit","c").replace("MooMooMeadows","mmm").replace("MushroomGorge","mg").replace("Toad'sFactory","tf").replace("Mario","m").replace("CoconutMall","cm").replace("DKSummit","dksc").replace("Wario'sGoldMine","wgm").replace("Daisy","d").replace("KoopaCape","kc").replace("MapleTreeway","mt").replace("GrumbleVolcano","gv").replace("DryDryRuins","ddr").replace("MoonviewHighway","mh").replace("Bowser's","b").replace("Castle","c").replace("RainbowRoad","rr").replace("Peach","p").replace("Beach","b").replace("YoshiFalls","yf").replace("GhostValley","gv").replace("Raceway","r").replace("SherbetLand","sl").replace("ShyGuy","sg").replace("DelfinoSquare","ds").replace("WaluigiStadium","ws").replace("DesertHills","dh").replace("Bowser","b").replace("DK","dk").replace("'sJungleParkway","jp").replace("Gardens","g").replace("Mountain","m");
